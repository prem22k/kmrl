...
const DocumentCard = memo(function DocumentCard({ document, onView, onDownload }) {
  ...
  return (
    <article
      className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 cursor-pointer group overflow-hidden"
      onClick={handleView}
      role="button"
      tabIndex={0}
    ...
    >
      ...
      <div className="flex items-start gap-3 mb-4">
        <div className="flex-shrink-0 bg-gradient-to-br from-blue-500 to-purple-600 p-2.5 rounded-lg shadow-md">
          <FileText className="w-5 h-5 text-white" aria-hidden="true" />
        </div>
        <div className="flex-1 min-w-0 overflow-hidden">
          <h3 
            className="font-semibold text-gray-900 truncate group-hover:text-blue-600 transition-colors text-sm mb-1" 
      ...
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        <span 
          className={`px-2.5 py-1 rounded-full text-xs font-medium border ${categoryColor} truncate max-w-[120px]`}
        ...
        <span 
          className={`px-2.5 py-1 rounded-full text-xs font-medium ${priority.color} bg-white border border-current flex items-center gap-1 flex-shrink-0`}
      ...
      <div className="mb-4 overflow-hidden">
        <p className="text-sm text-gray-600 leading-relaxed line-clamp-3" aria-label="Document analysis">
          {truncate(document.analysis, 120)}
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 pt-3 border-t border-gray-100">
        <button
          onClick={handleView}
          className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-lg hover:from-blue-600 hover:to-purple-700 transition-all duration-300 font-medium text-sm shadow-sm hover:shadow-md whitespace-nowrap"
        ...
        <button
          ...
          className="px-3 py-2 bg-white border-2 border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 hover:border-blue-400 transition-all duration-300 font-medium text-sm flex-shrink-0"
